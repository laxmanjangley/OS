...
sys_exofork(void)
{
	...
	struct Env *e;
	int x = env_alloc(&e, curenv->env_id);
	if(x) return x;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if(srcva>= (void*) UTOP || dstva>= (void*) UTOP || srcva != ROUNDDOWN(srcva,PGSIZE) || dstva != ROUNDDOWN(dstva,PGSIZE)) return -E_INVAL;
	int p = PTE_U | PTE_P;
	if((perm & p) != p) return -E_INVAL;
	pte_t *pte;
	struct PageInfo* pp = page_lookup(e1->env_pgdir,srcva,&pte);
	if(((perm & PTE_W) && (*pte & PTE_W) == 0)) return -E_INVAL;
	if(!pp) return -E_INVAL;
	x = page_insert(e2->env_pgdir,pp,dstva,perm);
	return x;
